...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    if (!op.equals("+")) { // 공백을 uri에서 +로 인식함 따라서 입력할 때 %2b로 해야한다
      // 자신의 일이 아니라면 다른 서블릿으로 위임할 수 있다.
      // => 요청을 다른 서블릿으로 전달할 수 있다.
      // 요청배달자 = request.getRequestDispatcher(다른 서블릿 URL);
      RequestDispatcher 요청배달자 = request.getRequestDispatcher("/ex07/s2"); // 포워딩하는 코드
