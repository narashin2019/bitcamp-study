// Object 클래스의 메서드를 오버라이딩 하기 - toString() // corelib ex01이 오리지널임
package com.eomcs.oop.ex06.x;
...
public class Exam0110 extends Object {
  ...
  static class Score /* extends Object */ {
    ...
    // 생성자
    public Score(final String name, final int kor, final int eng, final int math) {
      this.name = name;
      this.kor = kor;
      this.eng = eng;
      this.math = math;
  ...
  public static void main(final String[] args) {
    final Score s1 = new Score("홍길동", 100, 100, 100);

    final String str = s1.toString();
    // toString()?
    // => Score 클래스의 수퍼 클래스인 Object의 메서드 이다.
    ...
    // toString()의 리턴값?
    // => Object의 toStrong()을 호출하면 다음 형식의 문자열을 리턴한다.
    // "패키지 이름을 포함한 클래스명$안쪽클래스명@인스턴스 식별자(인스턴스주소아님)"
