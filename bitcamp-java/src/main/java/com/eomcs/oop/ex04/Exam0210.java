...
public class Exam0210 {
    public static void main(String[] args) throws Exception {
        ...
        char c = s1.charAt(1); //s1이 가리키는 문자셋에서 charAt(1)1번문자 찾아서 출력하라 // 인스턴스마다 달리 동작하는! 인스턴스 메서드!
        System.out.println(c);
        
        System.out.println(s1.compareTo("Helli")); //s1인스턴스와 compareTo()를 비교한다. 인스턴스 메서드
        System.out.println(s1.compareTo("Hello"));
        System.out.println(s1.compareTo("Hellu"));
        
        System.out.println(s1.contains("ll")); //인스턴스메서드. 인스턴스주소s1에 있는 객체에 ()안에 것이 포함되어 있느냐
        System.out.println(s1.contains("ee"));
        
        // 두 문자열을 연결하여 새 문자열을 만들자!
        String s2 = s1.concat(", world!"); //s1은 바뀌지 않음.
        ...
        // 즉 "클래스 메서드=스태틱 메서드"를 제공한다.(클래스이름으로호출)
        
        // => 형식을 갖춘 문자열을 만들기***
        String s4 = String.format("%s님 반갑습니다", "홍길동"); //printf와의 차이 printf는 출력만. 여기는 값을 리턴함!
