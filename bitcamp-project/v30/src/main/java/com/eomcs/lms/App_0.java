...
public class App_0 {
  ...
  private static void loadLessonData() {
    ...
    File file = new File("./lesson.ser"); // 파일명 바꿈 ser

    try (ObjectInputStream in =
        new ObjectInputStream(new BufferedInputStream(new FileInputStream(file)))) { //Object로 바꿈
      
      int size = in.readInt();
      
      for (int i = 0; i < size; i++) {
        lessonList.add((Lesson) in.readObject()); // 형변환 꼭 해주고, 임시변수 없앰
      }
      
      System.out.printf("총 %d 개의 수업 데이터를 로딩했습니다.\n", lessonList.size());

    } catch (Exception e) { // 그냥 Exception으로 바꿈
