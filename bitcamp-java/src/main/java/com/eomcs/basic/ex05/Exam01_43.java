...
public class Exam01_43 {
  public static void main(String[] args) {
    ...
    System.out.println(r1); // 988.7654출력
    // 기대값: 987.6543 + 1.111111 = 988.765411
    // 결과값: 987.6543 + 1.111111 = 988.7654(11)
    // 결과가 옳지 않게 나온 이유?
    // => float과 float의 연산결과는 float이이다.
    // => 그래서 메모리 크기를 넘어가는 뒤의 11은 짤린다.
        
    
    //그럼 결과를 담을 변수의 크기를 늘리면 되는가?
    double r2 = f1 + f2;
    System.out.println(r2); // 988.765380859375출력
    // 기대값: 988.765411
    // 결과값: 988.765380859375
    // 기대한 결과가 나오지 않은 이유?
    // => float과 float의 연산결과는 float이다.
    // => double 변수에 저장하기 전에 이미 float값이 되면서 일부 값이 돼곡 되었다.

    // 그런데 r1변수와 달리 뒤에 이상한 숫자가 많이 붙는 이유는 무엇인가?
    // => IEEE 754의 이진수 변환 문제때문이다.
    // => 4바이트 float 부동소수점을 8바이트 double 부동소수점 변수에 저장할 때 왜곡된 값이 들어 갈 수 있다.***
    ...
    double d1 = 987.6543;
    double d2 = 1.111111;
    double r5 = d1 + d2; // = 988.765411 기대
    System.out.println(r5); // 988.7654110000001 출력
    // 그럼에도 실제 출력해보면 맨 뒤에 극한의 작은 수가 붙는다
    // 이유? IEEE 754 이진 수 변환 문제이다. 고민하지 말라!
    // 어떻게 처리할 건데?
    // 맨뒤에 붙는 극한의 작은 수는 그냥 잘라 버린다.
