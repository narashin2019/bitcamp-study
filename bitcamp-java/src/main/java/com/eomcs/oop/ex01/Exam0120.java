...
public class Exam0120 {
  public static void main(String[] args) {
    ...
    //    클래스 문법으로 설계한 메모리는 이런방식으로 만들수 없다.
    
    // class 문법으로 설계한 메모리 준비하기
    // => new 클래스명();
    
    s = new Score(); 
    // => Score 설계도에 따라 메모리를 사용할 수 있도록 확보한다.
    // => 이렇게 설계도에 따라 준비긴 메모리를 "인스턴스(instance)"라 부른다
    // => 확보된 메모리를 사용하려면 주소를 보관할 필요가 있다.
    // => 위에서 준비한 레퍼런스에 주소를 보관한다.
    // 그 메모리의 주소를 s에 저장한다.   
    ...
    // => 다시 줄여서 "s 인스턴스의 name 값" (정확히는 s는 객체가 아니고 레퍼런스다.)
    // => 다른 말로 "s 객체의 name 값" (정확히는 s는 객체가 아니고 레퍼런스다.)
