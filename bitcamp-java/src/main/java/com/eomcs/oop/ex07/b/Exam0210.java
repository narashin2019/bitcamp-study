...
public class Exam0210 extends C {
  ...
  public static void main(String[] args) {
    ...
    //obj1.privateVar = 100; // 접근 불가! 오직 그 클래스 안에서만 사용가능.(해당클래스의 인스턴스멤버만접근가능)
    obj1.defaultVar = 100; // OK! 이 클래스는 A 클래스와 같은 패키지에 소속되어 있다.(캍은 패키지에 속한 클래스의 멤버라면)_
    ...
    // 자식 클래스인데 접근 불가?
    // 이유 => 자기의 인스턴스 변수가 아니다. c클래스를 통해 만든거지 exam210을 통해 만들지 않음
    obj2.publicVar = 100; // OK! 모두 다 접근 가능.
