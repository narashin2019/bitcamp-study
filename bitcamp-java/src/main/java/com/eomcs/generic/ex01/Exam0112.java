...
public class Exam0112 {
  ...
  static class ObjectBox {
    ...
    // 그런데 원래의 목적과 다르게 다음과 같이 Member가 아닌 String 객체를 넣어도 막을 방법이 없다.
    box1.set(new String("hello"));
    
    // 이유?
    // => set의 파라미터 타입은 Object이기 때문이다.
    // => Object는 모든 자바 객체(타입)를 받을 수 있다. 
    //
    // 이렇게 Box 클래스의 인스턴스 변수를 Object로 선언하면
    // 다양한 객체를 보관할 수는 있지만 
    // MemberBox나 StringBox, IntegerBox 처럼 특정 타입의 객체로 제한할 수 없다.
    // 그렇다고 이전처럼 각 타입별로 Box클래스를 정의하는 것은 바람직하지 않다.
    //
    ...
    // => Exam.0113.java를 확인해보라!
    
    // 변수 선언 시 변수 타입이나 리턴 타입을 특정 타입으로 제한하면서도 클래스는 한 개만 정의할 수 있다.
