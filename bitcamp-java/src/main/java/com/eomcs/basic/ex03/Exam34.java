...
// 
//- 정수처럼 메모리 크기(4바이트 또는 8바이트)에 따라 표현할 수 있는 
//  부동소수점의 범위가 다르다.
...
//
  public class Exam34 {
    public static void main(String[] args) {
      ...
      //## 4byte(float) 부동소수점 유효 자릿수 : 소수점을 뺀 7자리 (100%는 아님, 거의 완벽하다)**
      //float f =314.45678f; 부동소수점 컴파일시 에러 안떠도 유효자리수는 7자리임
      ...
      //## 8byte(double) 부동소수점 유효자리수 : 소수점을 뺀 15자리 (100%는 아님, 거의 완벽하다)**
      System.out.println(3.141592653589793); // OK!
      System.out.println(31415926.53589793); // OK! 3.141592653589793E7로출력
      System.out.println(314159265358979.3); // OK! 3.141592653589793E14

      //16자리인 경우 일부 부동소수점의 값이 제대로 저장되지 않는다
      System.out.println(914159265358979.3); // 값이 구겨진다. 9.141592653589792E14
      System.out.println(91415926.53589793); // OK! 9.141592653589793E7
      System.out.println(9.141592653589793); // OK!

      //15자리인 경우 대부분 부동소수점의 값이 제대로 저장된다.
      System.out.println(914159265358979.9); // OK! 9.141592653589799E14
      System.out.println(91415926.53589793); // OK! 9.141592653589793E7
      System.out.println(9.141592653589793); // OK! 9.141592653589793
      ...
      // - 해결책
      // - 시스템에서 필요한 만큼 소수점 이하 자리수를 적절히 짤라 사용하라! (보정작업해라)

    }
  }


// 부동소수점을 2진수로 변환하는 것 = "정규화(nomalized)"
// 부동소수점을 2진수로 변환하는 규칙 = "IEEE 754" = 정규화 규칙을 정의한 문서


//## 결론!
//- 32비트(float) 메모리에 부동소수점을 저장할 때는 유효자릿수 7자리까지는 거의 가능하다. ex. 유효자리수까지는 완벽해!(x) 거의완벽(o)
//- 64비트(double) 메모리에 부동소수점을 저장할 때는 유효자릿수 15자리까지는 거의 가능하다.
//- 그래서 32비트 float을 단정도(single-precision)라 부르고,
//  64비트는 두 배 정밀하게 값을 저장한다고 해서 배정도(double-precision)이라 부른다. 배로 정밀한 정도
//
//## 메모리 비트와 2진수
...
//
//## 메모리에 값을 저장하기
//- 메모리는 비트로 되어 있기 때문에 어떤 유형의 값이든 2진수로 변환할 수 있다면 
//  메모리에 저장할 수 있다.
//- 예) 빛을 저장하기
//  - 카메라 렌즈로 들어온 빛을 메모리에 저장하려면 2진수로 변환해야 한다. R(0~255)G(0~255)B(0~255)
//  - 이런 역할을 하는 것이 이미지 센서이다.
//- 예) 소리를 저장하기
//  - 소리도 마찬가지이다. 메모리에 저장하려면 2진수로 변환해야 한다.
//  - 마이크로 들어온 음파를 ADC를 통해 디지털 신호, 즉 2진수로 변환한다.
//- 냄새를 전달하는게 아님. 냄새를 센서로 읽어서 2진수로 저장 보냄 저쪽에서 2진수가 냄새로 재생성.
//
//## 부동소수점이 메모리에 저장되는 원리
...
//- 가수부는 'Sign-Magnitude 방식'으로 변환하고, 지수부는 'Excess-K 방식'으로 변환한다. cf. 정수 중 음수는 2의보수법 사용해서 2진법으로 정규화
//
// ### IEEE-754
...
//   1100.011(2진수)
//   = 1.100011(2진수) * 2^3 ***
//   가수부 => 100011(2진수) (앞에 1은 버립니당.. 전부 다 1일테니)
...
//유효자리수라도! 0.00000....1 의 오차가 있다.
//그래서 부동소수점은 정수와 다르게 정확하게 메모리에 저장되지 않는다.
//자리수 늘어날수록 정확도 확률이 점점낮아짐
