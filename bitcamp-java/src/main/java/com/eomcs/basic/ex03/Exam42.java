...
//- 자바는 메모리에 문자를 저장할 때는 'UCS(국제 문제 집합)'를 사용하고, 
//  외부로 입출력 할 때는 'UTF'를 사용한다.
//
...
//
  public class Exam42 {
    public static void main(String[] args) {
      ...
      System.out.println(0xac00); // 44032출력
      // => 그냥 출력하면 일반 숫자(16진수로 표현한 정수 리터럴이구나)를 표현한 것으로 오해한다.
      //    println()에 전달하는 값이 일반적인 수가 아니라 문자 코드임을 지정해야 한다.
      // => 숫자 앞에 문자 코드 임을 알리는 표시를 하라
      //    (char)0X0041
      // => 야 JVM 이거 정수 아니다. 문자 코드다. 
      //    그러니 이 문자 코드에 해당하는 문자를 폰트파일에서 찾아서
      //    그 코드에 해당하는 문자 그림을 출력해라.
      //    숫자 앞에 (char)를 붙여 이 숫자가 평범한 숫자가 아니라 문자의 UCS-2 코드 값임을 알려줘라.
      //    (char)문자코드 
      System.out.println((char)0x0041); // 앞에 00 생략 가능, A 출력
