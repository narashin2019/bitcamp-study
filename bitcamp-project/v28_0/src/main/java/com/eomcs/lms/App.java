...
public class App {
  ...
  private static void loadLessonData() {
    ...
    FileReader in = null; // 파일을 읽어들이는 빨대
    Scanner dataScan = null;

    try {
      // 파일을 읽을 때 사용할 도구를 준비한다.
      in = new FileReader(file); // 빨대는 file에 꼽는다.
      ...
      //
      dataScan = new Scanner(in); // 파일리더빨대에 콤마단위로 착즙?되는 스캐너를 붙인다.
      ...
      while (true) {
        ...
        } catch (Exception e) { // 반복문을 돌다가 읽어 들일게 없으면 반복문을 break한다.
          break;
        }
      }
      ...
      try {
        in.close();
      } catch (Exception e) { // IOException은 널포인트익셉션 처리 못한다
        // close() 실행 하다가 오류가 발생한 경우 무시한다.
    ...
    try {
      ...
      while (true) {
        try {
          ...
          member.setRegisteredDate(Date.valueOf(data[6])); // 모르겠다!!!!!!!!!
